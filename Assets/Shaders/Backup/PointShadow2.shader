Shader "Custom/Point Shadow2"
{
    ...
    float pcss(float3 uv, float objectDepth)
    {
        ...
        if (blockerDepth >= objectDepth - _CubeBias)return 1.0f;
        //return blockerDepth;
    ...
    float cssmBasis(samplerCUBE cubeMap1, samplerCUBE cubeMap2, float3 uv, float objectDepthZ, float filterSize)
    {
        ...
        if (supress_flag == 1.0)rec = SCALEFACTOR * (rec - OFFSET);
    ...
    float css(samplerCUBE cubeMap0, samplerCUBE cubeMap1, samplerCUBE cubeMap2, samplerCUBE cubeMap3, samplerCUBE cubeMap4,float3 uv, float objectDepth)
    {
